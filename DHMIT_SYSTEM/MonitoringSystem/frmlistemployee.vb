Public Class frmlistemployee
    ...
    Private Sub btnpreviewAll_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Form2.Show()
        Form2.Focus()
        sql = "SELECT  e.`EMPID`, concat( `emp_fname`,' ', `emp_lname`,' ', `emp_mname`) as 'Name' ,`emp_sex`,(`d_rate` * 14) as 'TwooWeeksSalary', `position`, `SUBJECT_HANDLED` FROM `employee` e ,`employee_workinfo` we  WHERE e.`EMPID`=we.`EMPID`"
    ...
    Private Sub btnpreviewSelected_Click(ByVal sender As System.Object, ByVal e As System.EventArgs)
        ...
        Form2.Show()
        Form2.Focus()
        sql = "SELECT  e.`EMPID`, `emp_fname`, `emp_lname`, `emp_mname`,`address`, `contact`, `status`, `emp_sex`, round( ((DATEDIFF( NOW( ) ,  `birth_date` ) /12) /31))  AS 'Age',`d_rate`, `position`, `d_hired`,(`d_rate` * 15) as 'Salary', `SUBJECT_HANDLED`, `REMAININGLEAVE`, `DEFAULTLEAVE`  ,w_type" &
