...
Public Class PDF_1
    ...
    Public Sub view()
        Try
            con.Open()
            sql = "SELECT * FROM tblfiles WHERE File_ID=" & DataGridView1.CurrentRow.Cells(0).Value
    ...
    Private Sub PDF_1_Load(sender As Object, e As EventArgs) Handles MyBase.Load
    ...
    Private Sub Timer1_Tick_1(sender As Object, e As EventArgs) Handles Timer1.Tick
        ...
        Call PDF_1_Load(sender, e)

    End Sub

    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        'sql = "DELETE FROM tblfiles WHERE File_ID = '" & DataGridView1.CurrentRow.Cells(0).Value & "'"
        ' createNoMsg(sql)
        sql = "DELETE FROM tblfiles WHERE File_ID = '" & DataGridView1.CurrentRow.Cells(0).Value & "'"
        deletes(sql, DataGridView1.CurrentRow.Cells(1).Value)

        sql = "SELECT `File_ID` AS 'File Id', `FileName` AS 'File Name' FROM `tblfiles`"
        reloadDtg(sql, DataGridView1)
    End Sub

    Private Sub TextBox2_TextChanged(sender As Object, e As EventArgs) Handles TextBox2.TextChanged
        sql = "SELECT `File_ID` AS 'File Id',`FileName` AS 'filename' FROM `tblfiles` WHERE `File_ID` LIKE '%" & TextBox2.Text & "%'" _
