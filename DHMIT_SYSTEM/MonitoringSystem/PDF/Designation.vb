...
Public Class Designation
    ...
    Public Sub view()
        Try
            con.Open()
            sql = "SELECT * FROM tbldesignation WHERE File_ID=" & DataGridView1.CurrentRow.Cells(0).Value
            cmd = New MySqlCommand
            With cmd
                .Connection = con
                .CommandText = sql
    ...
    Private Sub Timer1_Tick(sender As Object, e As EventArgs) Handles Timer1.Tick
        Try
            ...
            If ProgressBar1.Value = 100 Then
                ...
                con.Open()
                sql = "INSERT INTO `tbldesignation` (`FileName`) VALUES ('" & System.IO.Path.GetFileName(TextBox1.Text) & "')"
        ...
        Call Designation_Load(sender, e)
    End Sub

    Private Sub TextBox2_TextChanged(sender As Object, e As EventArgs) Handles TextBox2.TextChanged
        sql = "SELECT `File_ID` AS 'File Id',`FileName` AS 'filename' FROM `tbldesignation` WHERE `File_ID` LIKE '%" & TextBox2.Text & "%'" _
& " OR FileName LIKE '%" & TextBox2.Text & "%'"
        reloadDtg(sql, DataGridView1)
    End Sub

    Private Sub Button3_Click(sender As Object, e As EventArgs) Handles Button3.Click
        sql = "DELETE FROM tbldesignation WHERE File_ID = '" & DataGridView1.CurrentRow.Cells(0).Value & "'"
        deletes(sql, DataGridView1.CurrentRow.Cells(1).Value)

        sql = "SELECT `File_ID` AS 'File Id', `FileName` AS 'File Name' FROM `tbldesignation`"
        reloadDtg(sql, DataGridView1)
    End Sub
